...
const BTN = 'mt-8 inline-flex items-center gap-3 bg-transparent border border-gold-400 text-gold-400 px-8 py-4 text-xs uppercase tracking-widest font-medium hover:bg-gold-400 hover:text-charcoal-900 transition-all duration-500';
...
const services = [
  {
    ...
    icon: CalendarHeart,
    description: 'From intimate gatherings to grand celebrations, our experienced coordinators manage every aspect of your event — timelines, vendors, logistics, and on-the-day execution — so you can be fully present in the moment. Enquire now for a consultation.',
    image: eventsCoord,
  },
  {
    id: 'decor-hire',
    title: 'Décor Hire',
    icon: Armchair,
    description: 'An extensive catalogue of décor items— chairs, tables, backdrops, centrepieces, lighting, and more — available for hire to transform any venue into a setting your guests will never forget.',
    image: gradBalloons,
  },
  {
    id: 'catering',
    ...
    description: 'Tailored menus crafted with care, from elegant plated dinners to generous buffet spreads. We provide full catering equipment and professional service to suit all events. Enquire now for a consultation.',
    image: meatballs,
  },
  {
    id: 'photography',
    title: 'Photography',
    icon: Camera,
    description: 'Capture every fleeting moment with our professional photography services. From candid guest reactions to carefully styled portraits, we preserve the story of your day in timeless imagery.',
    image: familyPic,
  },
];
            ...
            <div className="w-full lg:w-[65%] flex items-center px-0 sm:px-4 md:px-12 lg:px-20 py-8 lg:py-16">
              <div className="max-w-lg">
                ...
                {service.id === 'events-coordination' && (
                  <button onClick={scrollToContact} className={BTN}>Enquire Now</button>
                )}
                {service.id === 'decor-hire' && onViewCatalogue && (
                  <button onClick={onViewCatalogue} className={BTN}>See Catalogue</button>
                )}
                {service.id === 'catering' && (
                  <button onClick={scrollToContact} className={BTN}>Enquire Now</button>
                )}
                {service.id === 'photography' && onViewGallery && (
                  <button onClick={onViewGallery} className={BTN}>More Info</button>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="bg-charcoal-950 py-20 md:py-28 mt-20 md:mt-28">
        <div className="max-w-3xl mx-auto px-6 text-center">
          ...
          <p className="text-white/60 text-sm md:text-base leading-relaxed mb-10 max-w-xl mx-auto">
            Get in touch to discuss your event. All orders and queries are handled via email, WhatsApp, or phone.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <a href="mailto:b.r.maxwell00@gmail.com"
            ...
            <a href="https://wa.me/27813566058" target="_blank" rel="noopener noreferrer"
